...
class SplitAttention(nn.Module):
    def __init__(self, in_channels, num_paths, reduction=4):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv3d(in_channels // reduction, in_channels * num_paths, kernel_size=1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        batch_size, channels, _, _, _ = x.shape
        assert channels % self.num_paths == 0

        split_size = channels // self.num_paths
        x_splits = torch.split(x, split_size, dim=1)

        pooled_splits = [self.avg_pool(split) for split in x_splits]
        pooled = torch.cat(pooled_splits, dim=1)

        fc1_out = self.fc1(pooled)
        fc1_out = self.relu(fc1_out)
        fc2_out = self.fc2(fc1_out)

        attn_splits = torch.split(fc2_out, split_size, dim=1)
        attn_weights = [self.sigmoid(split) for split in attn_splits]

        out_splits = [split * attn_weight for split, attn_weight in zip(x_splits, attn_weights)]
...
class ResNeStBottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None, num_paths=2, reduction=4):
        super().__init__()
        ...
        self.downsample = downsample
        self.stride = stride
        self.split_attn = SplitAttention(planes * self.expansion, num_paths, reduction)
...
class ResNeSt(nn.Module):
    def __init__(self,
        ...
        self.layer1 = self._make_layer(
            block, block_inplanes[0], layers[0], shortcut_type, num_paths=num_paths, reduction=reduction)
        self.layer2 = self._make_layer(
            block, block_inplanes[1], layers[1], shortcut_type, stride=2, num_paths=num_paths, reduction=reduction)
        self.layer3 = self._make_layer(
            block, block_inplanes[2], layers[2], shortcut_type, stride=2, num_paths=num_paths, reduction=reduction)
        self.layer4 = self._make_layer(
            block, block_inplanes[3], layers[3], shortcut_type, stride=1, num_paths=num_paths, reduction=reduction)
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1, num_paths=2, reduction=4):
        downsample = None
        if stride!= 1 or self.in_planes!= planes * block.expansion:
            if shortcut_type == 'A':
                downsample = partial(self._downsample_basic_block,
        ...
        self.in_planes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.in_planes, planes, num_paths=num_paths, reduction=reduction))
...
def seresnet50(input_channels=1, num_classes=2):
    return ResNet(addse_Bottleneck, [3, 4, 6, 3],  get_inplanes(), n_input_channels=input_channels, n_classes=num_classes)

def resnest50(input_channels=1, num_classes=2, num_paths=8, reduction=4):
    return ResNeSt(ResNeStBottleneck, [3, 4, 6, 3], get_inplanes(), n_input_channels=input_channels,
                    n_classes=num_classes, num_paths=num_paths, reduction=reduction)
