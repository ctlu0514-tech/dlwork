...
parser.add_argument('--data_root', type=str,
                    default='/nfs/zjq/prostate_ISUP/dataset/data_8', help='path to train dataset')
...
parser.add_argument('--model', choices=['MobileNet', 'MobileNetV3_Small', 'MobileNetV3_Large', 'ShuffleNet', 'DenseNet', 'resnet10', 'resnet18', 'resnet34','resnet50','resnet101','C3D', 'CNN_3D','Att_CNN_3D','LeNet', 'AlexNet','vit','resnext50','botnet50','Resnext50','cotnet50','resnext101','vgg','SwinTransformer','Densenet36_fgpn','Mynetwork','MyNetworkWithSeparateInputs','MyNetworkWithSeparateInputs2','resnet50_with_5_modals','ResNet50WithCrossTransformer','ResNetWithCrossTransformerAndLoss','rest50','coupleMamba','Mamba','MutiCoupleMamba','net50','seresnet50','resnest50'], default='resnext50')
...
parser.add_argument('--input_channels', default=5, type=int, help="input_channels (2,3,5)")

parser.add_argument('--invol', action='store_true')
...
def main(args):
    ...
    args.save_folder = pathlib.Path(f"/nfs/zjq/prostate_ISUP/new_result/Dataset_crop_gamma/RandAffine_135"
                                    f"/{args.exp_name}")
    ...
    if model_cla is not None:
        optimizer_cla = torch.optim.Adam(model_cla.parameters(), lr=args.lr, weight_decay=0.1, eps=1e-4)
        #scheduler_cla = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_cla, 'min')
        scheduler_cla = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=50, eta_min=1e-8, last_epoch=-1)
        #scheduler_cla = None
    ...
    for epoch in range(args.epoch):
        ...
        if val_acc > best_acc:
            ...
            best_epoch = epoch + 1
            #print(f'目前最好的epoch是Epoch {best_epoch}')
        ...
        # 学习率预热参数
        warmup_epochs = 30
        warmup_factor = 0.1  # 预热期间的学习率倍数
        initial_lr = 0.01
        if epoch < warmup_epochs:
            # 计算预热学习率
            warmup_lr = initial_lr * warmup_factor * (epoch + 1) / warmup_epochs
...
def step(args, data_loader, model, loss_function, optimizer, epoch, batchsize, steps,
    ...
    with trange(len(data_loader)) as t:
        for i, batch in enumerate(data_loader):
            ...
            #inputs1 = batch['ADC'].cuda()
            #inputs2 = batch['DWI'].cuda()
            #inputs3 = batch['T2FS'].cuda()
            #inputs4 = batch['CT'].cuda()
            #inputs5 = batch['PET'].cuda()
            ...
            #outputs, loss_constraint = model(inputs1,inputs2,inputs3,inputs4,inputs5)
            predict = torch.max(outputs, dim=1)[1]
            _acc += torch.eq(predict, labels).sum().item()
            loss = loss_function(outputs, labels.long())
            #loss = loss + 0.1 * loss_constraint
